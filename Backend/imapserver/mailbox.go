...
func (mbox *Mailbox) getIDsFromSeqSet(uid bool, seqSet *imap.SeqSet) ([]int32, error) {
	fmt.Printf("Called getIDsFromSeqSet %+v\n", mbox)
	fmt.Printf("SeqSet: %+v, uid: %v\n", seqSet, uid)
	var ids []int32
...
func (mbox *Mailbox) Name() string {
	fmt.Printf("Called Name %+v\n", mbox)
	return mbox.name
}

func (mbox *Mailbox) Info() (*imap.MailboxInfo, error) {
	fmt.Printf("Called Info %+v\n", mbox)
...
func (mbox *Mailbox) Status(items []imap.StatusItem) (*imap.MailboxStatus, error) {
	fmt.Printf("Called Status %+v\n", mbox)
	fmt.Printf("Status items requested: %+v\n", items)
	...
	fmt.Println("Mailbox status:", status)
	return status, nil
}

func (mbox *Mailbox) SetSubscribed(subscribed bool) error {
	fmt.Printf("Called SetSubscribed %+v\n", mbox)
	return nil
}

func (mbox *Mailbox) Check() error {
	fmt.Printf("Called Check %+v\n", mbox)
	return nil
}

func (mbox *Mailbox) Expunge() error {
	fmt.Printf("Called Expunge %+v\n", mbox)
	return mbox.backend.Storage.MailExpunge(mbox.name)
}

func (mbox *Mailbox) UpdateMessagesFlags(uid bool, seqSet *imap.SeqSet, op imap.FlagsOp, flags []string) error {
	fmt.Printf("Called UpdateMessagesFlags %+v\n", mbox)
...
func (mbox *Mailbox) CopyMessages(uid bool, seqSet *imap.SeqSet, destName string) error {
	fmt.Printf("Called CopyMessages %+v\n", mbox)
	return nil
}

func (mbox *Mailbox) CreateMessage(flags []string, date time.Time, body imap.Literal) error {
	fmt.Printf("Called CreateMessage %+v\n", mbox)
	return nil
}

func (mbox *Mailbox) SearchMessages(uid bool, criteria *imap.SearchCriteria) ([]uint32, error) {
	fmt.Printf("Called SearchMessages %+v\n", mbox)
	return mbox.backend.Storage.MailSearch(mbox.name)
}

func (mbox *Mailbox) ListMessages(uid bool, seqSet *imap.SeqSet, items []imap.FetchItem, ch chan<- *imap.Message) error {
	fmt.Printf("Called ListMessages %+v\n", mbox)
	defer close(ch)
