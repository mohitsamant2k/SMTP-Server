...
const insertMailStmt = `
	INSERT INTO mails (mailbox, id, mail, datetime, sender, receiver) VALUES(
		$1, (
...
func NewTableMails(db *sql.DB, WithTx func(func(*sql.Tx) error) error) (*TableMails, error) {
	t := &TableMails{
		db :     db,
		withTx: WithTx,
	}
...
func (t *TableMails) MailCreate(mailbox string, data []byte, sender string, receiver string) (int, error) {
	var id int
	err := t.withTx(func(txn *sql.Tx) error {
		return t.createMail.QueryRow(mailbox, data, time.Now().Unix(), sender, receiver).Scan(&id)
	})
	return id, err
}

func (t *TableMails) MailSelect(mailbox string, id int) (int, *Mail, error) {
	var seq int
	var datetime int64
	mail := &Mail{}
...
func (t *TableMails) MailUpdateFlags(mailbox string, id int, seen, answered, flagged, deleted bool) error {
	return t.withTx(func(txn *sql.Tx) error {
